...
class CreateSlotgroupsService
  ...
  def perform
    @slotgroups_array = create_slotgroups # step 1.1.
    ...
    determine_slots_simulation_status # step 3.3.
  ...
  def put_slots_simulation_status_to_x(slots_hashes, true_or_false)
    slots_hashes.each do |slot_hash|
      @slots_array.select { |x| x[:slot_instance] == slot_hash[:slot_instance] }.each do |slot_hash_selected|
        slot_hash_selected[:simulation_status] = true_or_false
      end
    end
  end
  ...
  def nb_slots_to_simulate(slotgroup_id)
    a = @slots_array.find { |x| x[:slotgroup_id] == slotgroup_id && x[:simulation_status] == true }
    if a.nil?
      0
    elsif a.class == Array ? a.count : 1
    end
  end
    ...
    @slotgroups_array.each do |slotgroup|
      slotgroup.overlaps.each do |slotgroup_overlaps_hash|
        ...
        slotgroup_overlaps_hash[:users].each do |overlapping_user|
          ...
          # record changes made so that ressource doesn't get unavailable for both slotgroups
  ...
  def determine_slots_to_simulate
    @slotgroups_array.each do |slotgroup|
      slotgroup.slots_to_simulate = slots_to_simulate(slotgroup.id)
    end
  end

  def slots_to_simulate(slotgroup_id)
    @slots_array.select { |x| x[:slotgroup_id] == slotgroup_id && x[:simulation_status] == true }
  end

  def determine_combinations_of_available_users
    @slotgroups_array.each do |slotgroup|
      combinations_size = slotgroup.determine_combinations_size
      slotgroup.combinations_of_available_users = slotgroup.list_available_users.combination(combinations_size).to_a
      if slotgroup.combinations_of_available_users[0].empty?
        slotgroup.nb_combinations_available_users = 0
      else
        slotgroup.nb_combinations_available_users = slotgroup.combinations_of_available_users.count
      end
    end
  end
